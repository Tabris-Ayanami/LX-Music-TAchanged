  ...
  <material-popup-btn :class="$style.btnContent">
    ...
    <template #content>
      <div :class="$style.setting">
        <div :class="$style.value">{{ Math.trunc(volume * 100) }}%</div>
        ...
        <button
          :class="[$style.muteBtn, { [$style.muteActive]: isMute }]"
          :aria-label="$t('player__volume_mute_label')"
          @click.stop="saveVolumeIsMute(!isMute)"
        >
          {{ $t('player__volume_mute_label') }}
        </button>
      </div>
    </template>
  </material-popup-btn>
