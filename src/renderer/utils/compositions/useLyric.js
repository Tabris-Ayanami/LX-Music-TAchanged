...
export default ({ isPlay, lyric, playProgress, isShowLyricProgressSetting, offset }) => {
  ...
  const findLyricTarget = dom => {
    while (dom) {
      if (dom.time != null) return dom
      if (dom.classList?.contains?.('pre')) return dom
      if (dom.classList?.contains?.('line-content')) return dom.parentNode
      if (dom.classList?.contains?.('line')) return dom.parentNode
      if (dom.tagName == 'SPAN') return dom.parentNode?.parentNode ?? null
      dom = dom.parentNode
    }
    return null
  }
  ...
  const throttleSetTime = throttle(() => {
    if (!dom_skip_line.value || !dom_lyric.value) return
    const rect = dom_skip_line.value.getBoundingClientRect()
    const lyricRect = dom_lyric.value.getBoundingClientRect()
    point.x = lyricRect.left + lyricRect.width / 2
    point.y = rect.top + rect.height / 2
    let dom = document.elementFromPoint(point.x, point.y)
    if (!dom) return
    dom = findLyricTarget(dom)
  ...
  const handleWheel = (event) => {
    ...
    dom_lyric.value.scrollTop = dom_lyric.value.scrollTop + event.deltaY / 1.2
    startLyricScrollTimeout()
    setTime()
  }
