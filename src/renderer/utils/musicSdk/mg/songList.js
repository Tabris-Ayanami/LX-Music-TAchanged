...
export default {
  ...
  limit_song: 50,
  successCode: '000000',
  cachedDetailInfo: createLimitedCache(80, 30 * 60 * 1000),
  cachedUrl: createLimitedCache(80, 30 * 60 * 1000),
  sortList: [
    {
      name: '推荐',
      id: '15127315',
  ...
  getListDetailInfo(id, tryNum = 0) {
    ...
    const cachedDetailInfo = this.cachedDetailInfo.get(id)
    if (cachedDetailInfo) return Promise.resolve(cachedDetailInfo)
    ...
    return requestObj_listDetailInfo.promise.then(({ body }) => {
      ...
      this.cachedDetailInfo.set(id, cachedDetailInfo)
  ...
  async getDetailUrl(link, page, retryNum = 0) {
    ...
    if (location) {
      this.cachedUrl.set(link, location)
  ...
  getListDetail(id, page, retryNum = 0) { // 获取歌曲列表内的音乐
    ...
    if (/\/playlist[/?]/.test(id)) {
      ...
      const url = this.cachedUrl.get(id)
