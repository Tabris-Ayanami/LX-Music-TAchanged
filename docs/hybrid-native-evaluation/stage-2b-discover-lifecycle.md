...
下一项优先调查 Discover 图片输入尺寸。本轮 Discover 样本同时有 31 个图片节点，其中 12 张已加载图片的固有尺寸合计约 53,530,329 像素；这是比单次 GPU process 波动更直接的图片解码压力信号。后续应先做图片缩略尺寸的单变量 A/B，再决定是否保留额外复杂度。
